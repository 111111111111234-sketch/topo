...
def draw_topo(
    ...
    for edge in st.get("topo", {}).get("edges", []):
        ...
        ax.plot([pa[0], pb[0]], [pa[2], pb[2]], color="#6b7280", linewidth=1.0 if nav else 0.7, linestyle="-" if nav else "--", alpha=0.65, zorder=1)
...
def draw_frame(
    ...
    fig.suptitle(
        "Step odometry audit | {} | target: {} | action: {} | raw drift: {:.3f}/{:.3f}m | corrected: {:.3f}/{:.3f}m | closures: {} | structural merge: {} | corrected nodes: {}".format(
            goal_text,
