...
#
# Terminal 1: conda activate vllm_server && bash ETPNav/scripts/start_vllm.sh 8000
# Terminal 2: conda activate conftopo && bash scripts/run_goal_graphs_goat_soon.sh
...
echo ""
echo "=== Step 2/2: CLIP embed (GOAT + SOON val JSON) ==="
python scripts/precompute_goal_graphs.py \
  --embed-only \
  --no-train
