...
echo "[4] etpnav — ConfTopo-ETPNav / habitat-sim 0.1.7 + torch 2.8+cu128"
if ! "$CONDA" env list | awk '{print $1}' | grep -qx etpnav; then
    skip "etpnav (run Dockerfile.part4)"
elif env -u LD_LIBRARY_PATH "$CONDA" run -n etpnav python -c "
...
print('habitat', habitat.__version__)
" 2>/dev/null; then
    pass "etpnav environment"
else
    fail "etpnav environment"
fi
echo ""
