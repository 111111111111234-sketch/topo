...
class StopVerifier:
    ...
    def observe(
        ...
        if not vlm_report or not bool(vlm_report.get("fresh", False)):
            return False
        hit = self._single_frame_stop_hit(vlm_report, goal)
    ...
    def _single_frame_stop_hit(
        ...
        center = _bbox_center_x(bbox)
        centered = center is None or 0.25 <= center <= 0.75
        strong_visual_stop = (
            bbox_area >= 0.12
...
class GOATAgent(ConfTopoBaseAgent):
    ...
    def act_with_state_machine(
        ...
        *,
        ...
        self._observe_target_visibility(target_visible, nav_target)
        ...
        if self.state.mode == GOATState.SEARCH:
            ...
            if (
                ...
                and self.stop_verifier.should_enter_verify(vlm_report, goal)
            ):
