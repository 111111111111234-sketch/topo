...
class SOONConfTopoAdapter:
    ...
    def goal_from_graph_dict(self, data: Dict[str, Any]) -> GoalNode:
        ...
        row = nodes[0]
        return GoalNode(
            target_object=str(row.get("target_object", row.get("description", "target"))),
    ...
    def goal_from_episode(self, episode: Dict[str, Any]) -> GoalNode:
        ...
        description = instructions[-1] if instructions else episode.get("instr_id", "target")
        return GoalNode(
            target_object=str(description),
